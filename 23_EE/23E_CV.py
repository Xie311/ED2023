...
)  # 典型范围：-8（最暗）到 -1（较亮），或 0.1~0.001（秒）
...
while cap.isOpened():
    ...
    if contours_red:
        ...
        if M_red["m00"] != 0:
            x_red, y_red = int(M_red["m10"] / M_red["m00"]), int(
                M_red["m01"] / M_red["m00"]
            )
    ...
    if contours_green:
        ...
        if M_green["m00"] != 0:
            x_green, y_green = int(M_green["m10"] / M_green["m00"]), int(
                M_green["m01"] / M_green["m00"]
            )

    # 绘制蓝色圆圈（红色光斑）
    if x_red > 0 and y_red > 0:
        cv2.circle(
            display_frame, (int(x_red), int(y_red)), 6, (255, 0, 0), 2
        )  # 蓝色空心圆
    # 绘制紫色圆圈（绿色光斑）
    if x_green > 0 and y_green > 0:
        cv2.circle(
            display_frame, (int(x_green), int(y_green)), 6, (255, 0, 255), 2
        )  # 紫色空心圆
