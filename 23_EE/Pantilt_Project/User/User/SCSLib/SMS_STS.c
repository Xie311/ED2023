...
int WritePosEx(uint8_t ID, int16_t Position, uint16_t Speed, uint8_t ACC)
{
	...
	bBuf[0] = ACC;
	Host2SCS(bBuf+1, bBuf+2, Position);		//将uint16拆为两个uint8
	Host2SCS(bBuf+3, bBuf+4, 0);
	Host2SCS(bBuf+5, bBuf+6, Speed);
	
	return genWrite(ID, SMS_STS_ACC, bBuf, 7);			//写入0x41位置中，该地址为加速度地址（1byte），往后一次为目标位置（2），运行时间（2），运行速度（2）//但是运行时间是个啥玩意
}
...
int WheelMode(uint8_t ID,uint8_t mode)
{
