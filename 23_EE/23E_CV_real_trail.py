...
def detect_black_border(
    frame, min_area=100, min_side=20, max_aspect_ratio=5, min_inner_ratio=0.5
):
    """
    检测图像中的黑色矩形边框
    ...
    参数:
        ...
        min_inner_ratio: 内框相对于外框的最小边长比例(0-1)

    返回:
        processed_frame: 处理后的图像(带标记)
        border_points: 检测到的边框点(4个有序角点坐标), 未检测到返回None
                      顺序: [左上, 右上, 右下, 左下]
    """
    # 转换为HSV颜色空间
    ...
    for contour in contours:
        ...
        if len(approx) == 4:
            ...
            if w > min_side and h > min_side and aspect_ratio < max_aspect_ratio:
                ...
                # 计算矩形平均边长作为排序依据
                avg_side = (w + h) / 2
                rectangles.append((avg_side, rect))
    ...
    # 如果找到一个矩形
    if len(rectangles) == 1:
        border_points = rectangles[0][1]
        # 绘制矩形（红色）
        cv2.drawContours(
            processed_frame, [border_points.astype(int)], 0, (0, 0, 255), 2
        )
    ...
    # 如果找到两个矩形
        ...
        # 计算外框平均边长
        outer_width = np.linalg.norm(outer_rect[0] - outer_rect[1])
        outer_height = np.linalg.norm(outer_rect[0] - outer_rect[3])
        outer_avg_side = (outer_width + outer_height) / 2

        # 计算内框平均边长
        inner_width = np.linalg.norm(inner_rect[0] - inner_rect[1])
        inner_height = np.linalg.norm(inner_rect[0] - inner_rect[3])
        inner_avg_side = (inner_width + inner_height) / 2

        # 检查内框是否满足最小边长比例要求
        if inner_avg_side >= outer_avg_side * min_inner_ratio:
            # 计算两个矩形的平均点
            border_points = (outer_rect + inner_rect) / 2
            # 绘制外矩形（红色）
            cv2.drawContours(
                processed_frame, [outer_rect.astype(int)], 0, (0, 0, 255), 2
            )
            # 绘制内矩形（绿色）
            cv2.drawContours(
                processed_frame, [inner_rect.astype(int)], 0, (0, 255, 0), 2
            )
        else:
            # 内框太小，只使用外框
            border_points = outer_rect
            cv2.drawContours(
                processed_frame, [outer_rect.astype(int)], 0, (0, 0, 255), 2
            )
    ...
    if border_points is not None:
        for i, (cx, cy) in enumerate(border_points):
    ...
    return processed_frame, border_points
